...
def extract_words_old(text): # Old version of extract_words using iteration; ignore this
    ...
    """
    "*** YOUR CODE HERE ***"
    char_num, prev_word_num, word_num, extracted_words = 0, 0, 0, ['',]
    while char_num < len(text):
        if text[char_num] in ascii_letters:
            if prev_word_num == word_num:
                extracted_words[word_num] = extracted_words[word_num] + text[char_num]
            else:
                extracted_words.append(text[char_num])
                prev_word_num = word_num
        else:
            if prev_word_num == word_num:
                prev_word_num, word_num = word_num, word_num + 1
        char_num += 1
    return extracted_words

def extract_words(text): # Cleaner version of extract_words using a for statement instead of iteration
    """Return the words in a tweet, not including punctuation.
    ...
    for char in text:
        if char in ascii_letters:
            extracted_word += char
        elif len(extracted_word) > 0:
            extracted_words.append(extracted_word)
            extracted_word = ''
    if len(extracted_word) > 0:
        extracted_words.append(extracted_word)
    return extracted_words
...
def sentiment_value(s):
    ...
    ''' assert has_sentiment(s), 'No sentiment value' ''' # The assertion errors were stopping my average_sentiments function in its tracks
    "*** YOUR CODE HERE ***"
    if has_sentiment(s) == False:
        return "None"
    else:
...
def analyze_tweet_sentiment(tweet):
    ...
    False
    """
total_sentiment, num_words_with_sentiment = 0, 0
for word_number in range(0, len(tweet_words(tweet))):
    if sentiment_value(get_word_sentiment(tweet_words(tweet)[word_number])) == 'None': 1+1
    else:
        total_sentiment += sentiment_value(get_word_sentiment(tweet_words(tweet)[word_number]))
        num_words_with_sentiment += 1
if total_sentiment == 0:
    return 5
else:
    return make_sentiment(total_sentiment / num_words_with_sentiment)
...
def find_centroid(polygon):
    ...
    while i<len(polygon)-1:
        subtractionfactor=(polygon[i][0]*longitude(polygon[i+1])-
                           latitude(polygon[i+1])*longitude(polygon[i])) # Taken from centroid formula
        x +=(latitude(polygon[i])+ latitude (polygon[i+1]))*subtractionfactor
        y +=(longitude(polygon[i]) + longitude (polygon[i+1]))*subtractionfactor
        Area +=subtractionfactor
        i +=1
    if Area==0:
        return (latitude(polygon[0]), longitude(polygon[0]), 0)
    else:
        return (2*x/(6*Area), 2*y/(6*Area), abs(0.5*Area))
...
def group_tweets_by_state(tweets):
    ...
    for tweet in tweets:
        state = find_closest_state(tweet, state_centers)
...
def most_talkative_state(term):
    ...
    state_centers = {n: find_center(s) for n, s in us_states.items()}
    tgbs = group_tweets_by_state(tweets) # Stands for "Tweets, Grouped By State" for shortening purposes :P
    most_tweets = 0
...
tweetswithtexas = group_tweets_by_state(load_tweets(make_tweet, 'texas')) # For testing the function out
tweetswithobama = group_tweets_by_state(load_tweets(make_tweet, 'obama')) # Also for testing
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        total_sentiment, none_tweets = 0, 0
        for tweet_number in range(0, len(tweets_by_state[state])):
            if analyze_tweet_sentiment(tweets_by_state[state][tweet_number]) == None: # If the new tweet you're analyzing is devoid of sentiment, then ignore it and move on.
                if total_sentiment != None and total_sentiment != 0:
                    1+1
                else:
                    total_sentiment = None
                none_tweets +=1
            else:
                if total_sentiment == None:
                    total_sentiment = 0
                total_sentiment += analyze_tweet_sentiment(tweets_by_state[state][tweet_number])
        if total_sentiment == None:
            1+1 # Leaves that state out from the dictionary / does nothing
        else:
            averaged_state_sentiments[state] = (total_sentiment / (len(tweets_by_state[state]) - none_tweets))
...
def group_tweets_by_hour(tweets):
    ...
    for tweet in tweets:
        for i in range(0, 24):
            if tweet_time(tweet).hour == i:
                if i in tweets_by_hour:
